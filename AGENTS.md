...
- `codex` (snake_case: `exec_command`, `view_image`, `request_user_input`, `spawn_agent`, `followup_task`, `list_agents`.) Which names a session sends depends on the model's tool mode: under `code_mode_only` the only `tools[]` entry is the freeform `exec` and every other name lives in its description, while shell-mode models — which the `CPA` provider is set to, where `deepseek` stays at the code-mode default — declare `exec_command`, `write_stdin`, `apply_patch` and `view_image` as their own `tools[]` entries. The table keys on that shell-mode surface: `exec_command` -> `run_command` and `view_image` -> `view_file`; `exec`, `write_stdin` and `apply_patch` pass through, and `exec` is deliberately absent because it and `exec_command` both want `run_command`. Intentional pass-through: `wait`, `request_user_input_async`, `sleep`, `send_message`, `wait_agent`, `interrupt_agent`. `shell_command` is a `shell_type` catalog label, not a tool name.
