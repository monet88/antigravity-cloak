...
- **OpenAI Codex (`codex`)**: `exec_command`, `view_image`, `request_user_input`, `spawn_agent`, `followup_task`, `list_agents`. Intentional pass-through: `exec`, `wait`, `write_stdin`, `apply_patch`, `request_user_input_async`, `sleep`, `send_message`, `wait_agent`, `interrupt_agent`. Which names a session declares depends on the model's tool mode: `code_mode_only` models ship `apply_patch`, `exec_command`, `view_image` and the rest inside the `exec` description, while shell-mode models declare them as their own `tools[]` entries, which is the surface this table maps because the AGY-facing routed providers run in shell mode. `exec` is deliberately absent: it and `exec_command` both want `run_command`, and the reverse table is the inverse of this one, so a target cannot carry two sources. `shell_command` is a `shell_type` catalog label rather than a tool name.
