...
- **OpenAI Codex (`codex`)**: `exec`, `web_search`, `request_user_input`, `collaboration__spawn_agent`, `collaboration__followup_task`, `collaboration__list_agents`. Intentional pass-through: `wait`, `request_user_input_async`, `clock__sleep`, `collaboration__wait_agent`, `collaboration__interrupt_agent`, `collaboration__send_message`, and every helper that exists only as prose inside the `exec` description (`apply_patch`, `exec_command`, `write_stdin`, `view_image`, `tool_search`). Code mode is the default for every routed provider and this table keys on what such a session actually declares over opencodex's `openai-chat` adapter; namespaced children arrive flattened as `<namespace>__<child>`. `exec` is the sole entry point and therefore owns `run_command`. `shell_command` is a `shell_type` catalog label rather than a tool name.
