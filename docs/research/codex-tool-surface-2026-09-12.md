...
## Historical Cloaking Baseline (2026-09-12; Superseded by 2026-09-25 Amendment)

> [!NOTE]
> **SUPERSEDED HISTORICAL SECTION:**
> The summary, "shipped table", and pass-through claims below document the historical baseline captured on 2026-09-12 at v0.5.2.
> As of 2026-09-25 (v0.6.0, Issues #38 & #32):
> - Shell-mode declarations (`exec_command`, `apply_patch`, `write_stdin`, `view_image`) and dynamic MCP declarations are cloaked via request-scoped alias plans (`wp_*` and `wp_ext_<hash>`).
> - Collaboration tools are aligned with cross-client shared vocabulary (`wp_send_message`, `wp_list_workers`, `wp_collaboration_followup_task`).
> - Prose helpers inside `exec` description and payload bodies remain untouched data/prose.
>
> See **[Amendment: 2026-09-25](#amendment-2026-09-25-issue-38-parent-32--request-scoped-alias-plans)** below and **[CONTEXT.md](../../CONTEXT.md)** for authoritative current behavior.
...
Document maintenance & date convention: When the wire surface materially changes, capture new runtime evidence and either publish a new dated reference file updating all repository links, or append an authoritative dated in-file amendment (such as the 2026-09-25 amendment below) that explicitly supersedes historical sections while preserving the empirical evidence of prior captures.
...
## Amendment: 2026-09-25 (Issue #38, Parent #32 — Request-Scoped Alias Plans)

This amendment documents the evolved behavior implemented for Issues #38 and #32, superseding historical assumptions while preserving the 2026-09-12 runtime captures above as historical context:

1. **Protocol-Declared Shell Helpers**:
   - In shell mode, declarations occupying protocol tool-identity positions (`exec_command`, `apply_patch`, `write_stdin`, `view_image`) are no longer uncloaked pass-through.
   - `exec_command` maps to `run_command`, while `apply_patch`, `write_stdin`, and `view_image` map to stable shared aliases (`wp_apply_patch`, `wp_write_stdin`, `wp_view_image`) via `codexSharedAliases` in request-scoped alias plans.
   - Uncloaking on response and stream paths restores the exact declared source names (`exec_command`, `apply_patch`, etc.) strictly scoped to the active alias plan for that request, eliminating cross-mode collision.
   - During mode transitions, the current `tools[]` declaration surface owns reverse authority for `run_command` while history can carry the alternate entry point without causing false collisions or session failure.

2. **Code-Mode Helpers in Prose/Payload**:
   - Helper names mentioned inside the `exec` description text or within tool argument/body payloads remain untouched as prose/data. They are not rewritten on the request path, preventing upstream execution distortion.

3. **Dynamic & Top-Level MCP Declarations**:
   - Top-level dynamic `mcp__*` tools or undeclared tool names are no longer treated as unmanaged pass-through.
   - They receive deterministic neutral fallback aliases (`wp_ext_<hash>`) upon admission, shielding client tool surface details upstream while guaranteeing exact request-scoped reversal on return.
