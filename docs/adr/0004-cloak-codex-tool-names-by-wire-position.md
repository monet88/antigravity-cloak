...
6. **Keep the request-scoped reverse narrowing for Codex only.** It is what made it safe to key the table on one mode at a time: a shell-mode request that declares `exec_command` never receives a `run_command` restored to `exec`. Narrowing reads the **source** side of a pair first: a declared source name proves the body is pre-cloak, and only a body that declares no source name at all is read as executed, where the targets are the only remaining evidence of which pairs were applied. Both sides have to be readable because the two are read from different bodies — request interception sees the raw client body, while the response and stream interceptors are handed the executed body, which the host only republishes after the cloak rewrite. Reading the target side whenever it matches would instead hand a target the client declared natively back as a Codex source name it never declared; reading only the source side dropped the whole reverse on every executed body.
