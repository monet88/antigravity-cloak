...
4. **Keep detection counting `codexSourceIdentityInventory`, not the rename table**, and add the flattened collaboration spellings to that inventory. A code-mode request that declares `exec` plus namespace children and nothing else otherwise scores one hit and stays undetected.
...
6. **Keep the request-scoped reverse narrowing for Codex only.** It is what made it safe to key the table on one mode at a time: a shell-mode request that declares `exec_command` never receives a `run_command` restored to `exec`.
