...
6. **Keep the request-scoped reverse narrowing for Codex only.** It is what made it safe to key the table on one mode at a time: a shell-mode request that declares `exec_command` never receives a `run_command` restored to `exec`. Narrowing matches a declared name on **either side** of a pair. The two sides are read from different bodies: request interception sees the raw client body (source names), while the response and stream interceptors are handed the executed body, which the host only republishes after the cloak rewrite (target names). Matching the source side alone narrowed every executed-body reverse down to nothing, so no codex response or legacy-schema stream was reversed at all.
